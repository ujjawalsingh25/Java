...
// ---------------------------------------------------------------------
class Detect_Bipartite_Graph_BFS {
    public boolean check_Alternate_Color_BFS(int firstNode, int[] colorArr, ArrayList<ArrayList<Integer>> adjArr, int nodes) {
        Queue<Integer> quu = new LinkedList<>();
        quu.add(firstNode);
        colorArr[firstNode] = 0;
        ...
        while(!quu.isEmpty()) {
            ...
            for(int neighborNode: adjArr.get(currentNode)) {
                if(colorArr[neighborNode] == -1) {
                    colorArr[neighborNode] = 1 - colorArr[currentNode];
                    quu.add(neighborNode);
                }   
                else if(colorArr[neighborNode] == colorArr[currentNode])  return false;
            }
        }
        return true;
    }

    public boolean isBipartite_Graph_BFS(ArrayList<ArrayList<Integer>> adjArr, int nodes) {
        int colorArr[] = new int[nodes+1];
        for(int i=0; i<nodes; i++)  colorArr[i] = -1;    // initialize the array with all -1
        for(int i=1; i<=nodes; i++) {
            if(colorArr[i] == -1) {
                if(check_Alternate_Color_BFS(i, colorArr, adjArr, nodes) == false)  return false;
            }
        }
        return true;
    }
...
class Graph_13_Bipartite_Graph_BFS {
    public static void main(String[] args) {
        ...
        Detect_Bipartite_Graph_BFS detect_Bipartite_Graph = new Detect_Bipartite_Graph_BFS();
        boolean isBipartite_BFS = detect_Bipartite_Graph.isBipartite_Graph_BFS(graph.graph_ArrayList, nodes); 
        if(isBipartite_BFS)   System.out.println("True");
