...
// ---------------------------------------------------------------------
class Dijktras_Algorithm_usingPriorityQueue {
    public int[] Dijktras_Algorithm(int srcNode, ArrayList<ArrayList<ArrayList<Integer>>>  arr, int nodes) {
        int[] distArr = new int[nodes];
        PriorityQueue<Pair> prtQuu = new PriorityQueue<Pair>((x,y) -> x.distance - y.distance);
        
        for(int i=0; i<nodes; i++) {
            distArr[i] = (int)(1e9);
        }
        distArr[srcNode] = 0;
        prtQuu.add(new Pair(0, srcNode));
        while(!prtQuu.isEmpty()) {
            int prevDist = prtQuu.peek().distance;
            int node = prtQuu.peek().node;
            prtQuu.remove();
            
            for(int i=0; i<arr.get(node).size(); i++) {
                int weight = arr.get(node).get(i).get(1);
                int nbrNode = arr.get(node).get(i).get(0);
                if(prevDist + weight < distArr[nbrNode]) {
                    distArr[nbrNode] = prevDist + weight;
                    prtQuu.add(new Pair(distArr[nbrNode], nbrNode));
                }
            }
        }
        return distArr;
...
class Graph_25_Dijktras_Algorithm_usingPriorityQueue {
    public static void main(String[] args) {
        ...
        System.out.println("--------------------------------");
        Dijktras_Algorithm_usingPriorityQueue shortestPath = new Dijktras_Algorithm_usingPriorityQueue();
